...
const initialAlertData = [
  {
    ...
    exceededLimit: "1000 tablets",
    alertDate: "2024-08-10",
    status: "Critical",
  },
  {
    drugName: "Amoxicillin 250mg",
    exceededLimit: "500 capsules",
    alertDate: "2024-08-12",
    status: "High",
  },
  {
    drugName: "Insulin 100IU",
    exceededLimit: "200 vials",
    alertDate: "2024-08-15",
    status: "Moderate",
  },
  {
    drugName: "Aspirin 75mg",
    exceededLimit: "800 tablets",
    alertDate: "2024-08-17",
    status: "Low",
  }
];

function InstituteAlertsTable() {
  ...
  const handleDownload = () => {
    ...
    link.href = url;
    link.setAttribute('download', 'drug_alerts.csv');
  ...
  const handleSubmitAppeal = () => {
    if (appealFile) {
      Papa.parse(appealFile, {
        header: true,
        skipEmptyLines: true,
        complete: function (results) {
          // Handle the appeal submission logic here
          console.log('Appeal Data:', results.data);
        },
      });
    }
  };
  ...
  return (
    <div className={`${poppins.className} relative overflow-x-auto mt-10 mx-20`}>
      ...
      <h2 className="text-3xl font-semibold pb-6">Appeal the Decision</h2>
      <div className="mb-4">
      ...
      <table className="w-full text-sm text-left text-gray-500">
        <thead className="text-md text-gray-700 bg-gray-50 border-b">
          <tr>
            ...
            <th scope="col" className="px-2 py-3">Exceeded Limit</th>
            <th scope="col" className="px-2 py-3">Alert Date</th>
          ...
          {alerts.map((alert, index) => (
            <tr key={index} className="bg-white border-b">
              ...
              <td className="px-2 py-4">
                <h1 className="truncate w-56">{alert.drugName}</h1>
              </td>
              <td className="px-2 py-4">
                <h1 className="truncate w-56">{alert.exceededLimit}</h1>
              </td>
              <td className="px-2 py-4">
                <h1 className="truncate w-56">{alert.alertDate}</h1>
              </td>
              <td className="px-2 py-4">
                <span className={`inline-block px-3 py-1 text-sm font-semibold text-white rounded-full ${getStatusBadge(alert.status)}`}>
                  {alert.status}
                </span>
              </td>
            </tr>
...
export default InstituteAlertsTable;
